...
Providers that cannot furnish a separately identified context must record `Independent adjudication: not available`, set `review_confidence=low`, and cap the verdict at `conditional`/YELLOW. GREEN is unreachable without confirmation. Silence is not agreement. OpenCode and Codex ship no bundled verifier of their own, but they no longer have to: the engine spawns each role as its own process, so an adjudicator can be any CLI you name, and a different one from the reviewer. See `roles.md`. A grind caps at YELLOW when no independent adjudication was obtained, not because of which provider you started from.
